-- DEFINITIVE SIMULATION FUNCTION — ALL 9 PARTIES
-- Drop and recreate the simulation function with all Nigerian parties.
-- Also fix the party_results delete to use TRUNCATE with lock_timeout.
...
BEGIN
  ...
  -- Clear old data in batches (avoids TRUNCATE lock timeout)
  WHILE EXISTS (SELECT 1 FROM party_results LIMIT 1) LOOP
    DELETE FROM party_results WHERE id IN (SELECT id FROM party_results LIMIT 50000);
    COMMIT;
  END LOOP;

  WHILE EXISTS (SELECT 1 FROM result_submissions LIMIT 1) LOOP
    DELETE FROM result_submissions WHERE id IN (SELECT id FROM result_submissions LIMIT 50000);
    COMMIT;
  END LOOP;

  DELETE FROM incidents WHERE id IN (SELECT id FROM incidents LIMIT 10000);
  ...
  vote_data AS (
    SELECT ri.id AS result_id, pd.region, ri.valid_votes,
      ...
      -- Remaining votes for other 7 parties
