-- Fix lock timeout: previous simulation's TRUNCATE holds locks too long
-- Add SET lock_timeout = '60s' to the simulation function
...
BEGIN
  ...
  -- Delete in batches to avoid long locks from TRUNCATE
  ...
  SELECT vd.result_id, op.id,
    GREATEST(0, ROUND((vd.valid_votes - vd.ndc_votes - vd.apc_votes) *
      (CASE op.rn
        WHEN 1 THEN 0.30 WHEN 2 THEN 0.20 WHEN 3 THEN 0.12
        WHEN 4 THEN 0.10 WHEN 5 THEN 0.08 WHEN 6 THEN 0.10 WHEN 7 THEN 0.10
