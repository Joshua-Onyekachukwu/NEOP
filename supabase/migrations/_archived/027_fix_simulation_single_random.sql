-- Fix: use a single random() call per row to compute rejected, then valid = total - rejected
...
BEGIN
  ...
  -- Use a single random() per row: compute reject_rate once, derive both columns from it
  WITH pu_data AS (
