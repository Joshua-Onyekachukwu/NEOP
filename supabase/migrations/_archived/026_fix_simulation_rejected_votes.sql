-- Fix simulation: compute rejected_votes once, then valid = total - rejected
-- The previous version called ROUND(random()) twice getting different values
...
BEGIN
  ...
  pu_with_votes AS (
    SELECT *, 
      ROUND(total_votes * (0.01 + random() * 0.04))::INTEGER AS rejected,
      total_votes - ROUND(total_votes * (0.01 + random() * 0.04))::INTEGER AS valid
    FROM pu_data
  ),
  result_insert AS (
    ...
    SELECT pw.pu_id, v_election_id, NULL, NULL,
      pw.valid, pw.rejected, pw.total_votes,
    ...
    FROM pu_with_votes pw RETURNING id, polling_unit_id, total_votes, valid_votes
  ),
  vote_data AS (
