-- Fix simulation: rejected_votes must equal total_votes - valid_votes
-- to satisfy chk_votes_math constraint
...
BEGIN
  ...
  result_insert AS (
    ...
    SELECT pd.pu_id, v_election_id, NULL, NULL,
      -- valid_votes = total - rejected (rejected is 1-4% of total)
      pd.total_votes - ROUND(pd.total_votes * (0.01 + random() * 0.04))::INTEGER,
      -- rejected_votes = 1-4% of total
