-- Fix ALL remaining NOT NULL constraints on result_submissions that block simulation

-- rejected_votes: simulation sets it to NULL initially, then computes it
ALTER TABLE result_submissions ALTER COLUMN rejected_votes DROP NOT NULL;

-- election_id: should be nullable for simulation data
ALTER TABLE result_submissions ALTER COLUMN election_id DROP NOT NULL;

-- Also ensure the simulation function computes rejected_votes directly
-- instead of setting NULL then UPDATE in a separate CTE
...
BEGIN
  ...
  result_insert AS (
    ...
    SELECT pd.pu_id, v_election_id, NULL, NULL,
      ...
      ROUND(pd.total_votes * (0.01 + random() * 0.04))::INTEGER,
      pd.total_votes,
  ...
  result_fix AS (
    ...
    SET valid_votes = ri.total_votes - rs.rejected_votes
    FROM result_insert ri WHERE rs.id = ri.id RETURNING rs.id
  ),
  vote_data AS (
    SELECT ri.id AS result_id, pd.region, ri.total_votes, ri.valid_votes,
