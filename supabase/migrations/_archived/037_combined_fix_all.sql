-- ============================================================
-- 037_COMBINED_FIX.sql — Run this ONE file in Supabase SQL Editor
-- Combines migrations 033, 034, 035, 036 into safe chunks.
-- If it fails partway through, you can re-run safely (uses IF NOT EXISTS / ON CONFLICT).
-- ============================================================

-- ============================================================
-- CHUNK 1: Add is_active to elections (migration 034)
-- ============================================================
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'elections' AND column_name = 'is_active') THEN
    ...
    RAISE NOTICE 'Added is_active column to elections';
  ELSE
    RAISE NOTICE 'elections.is_active already exists';
  END IF;
END $$;

-- Mark all existing elections as active
UPDATE elections SET is_active = true WHERE is_active IS NULL;

-- ============================================================
-- CHUNK 2: Ensure all 9 parties exist
-- ============================================================
...
-- ============================================================
-- CHUNK 4: Indexes for fast queries (migration 032)
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_result_submissions_status ON result_submissions(status);
CREATE INDEX IF NOT EXISTS idx_result_submissions_polling_unit ON result_submissions(polling_unit_id);
CREATE INDEX IF NOT EXISTS idx_result_submissions_submitted ON result_submissions(submitted_at DESC);
CREATE INDEX IF NOT EXISTS idx_party_results_submission ON party_results(result_submission_id);
CREATE INDEX IF NOT EXISTS idx_party_results_party ON party_results(party_id);
CREATE INDEX IF NOT EXISTS idx_polling_units_state ON polling_units(state_id);
CREATE INDEX IF NOT EXISTS idx_polling_units_status ON polling_units(status);

-- ============================================================
-- CHUNK 5: Simulation function with ALL 9 parties
-- Run this as a separate statement if the dashboard times out
-- ============================================================
...
BEGIN
  ...
  UNION ALL
  SELECT vd.result_id, v_pdp_party_id, GREATEST(0, ROUND((vd.valid_total - vd.ndc_votes - vd.apc_votes) * 0.30 * (0.7 + random() * 0.6)))::INTEGER FROM vote_data vd WHERE (vd.valid_total - vd.ndc_votes - vd.apc_votes) > 0
  UNION ALL
  SELECT vd.result_id, v_lp_party_id, GREATEST(0, ROUND((vd.valid_total - vd.ndc_votes - vd.apc_votes) * 0.20 * (0.7 + random() * 0.6)))::INTEGER FROM vote_data vd WHERE (vd.valid_total - vd.ndc_votes - vd.apc_votes) > 0
  UNION ALL
  SELECT vd.result_id, v_nnpp_party_id, GREATEST(0, ROUND((vd.valid_total - vd.ndc_votes - vd.apc_votes) * 0.12 * (0.7 + random() * 0.6)))::INTEGER FROM vote_data vd WHERE (vd.valid_total - vd.ndc_votes - vd.apc_votes) > 0
  UNION ALL
  SELECT vd.result_id, v_apga_party_id, GREATEST(0, ROUND((vd.valid_total - vd.ndc_votes - vd.apc_votes) * 0.10 * (0.7 + random() * 0.6)))::INTEGER FROM vote_data vd WHERE (vd.valid_total - vd.ndc_votes - vd.apc_votes) > 0
  UNION ALL
  SELECT vd.result_id, v_sdp_party_id, GREATEST(0, ROUND((vd.valid_total - vd.ndc_votes - vd.apc_votes) * 0.08 * (0.7 + random() * 0.6)))::INTEGER FROM vote_data vd WHERE (vd.valid_total - vd.ndc_votes - vd.apc_votes) > 0
  UNION ALL
  SELECT vd.result_id, v_ypp_party_id, GREATEST(0, ROUND((vd.valid_total - vd.ndc_votes - vd.apc_votes) * 0.10 * (0.7 + random() * 0.6)))::INTEGER FROM vote_data vd WHERE (vd.valid_total - vd.ndc_votes - vd.apc_votes) > 0
  UNION ALL
  SELECT vd.result_id, v_adc_party_id, GREATEST(0, ROUND((vd.valid_total - vd.ndc_votes - vd.apc_votes) * 0.10 * (0.7 + random() * 0.6)))::INTEGER FROM vote_data vd WHERE (vd.valid_total - vd.ndc_votes - vd.apc_votes) > 0;
...
-- ============================================================
-- CHUNK 6: State breakdown function
-- ============================================================
...
BEGIN
  ...
  RAISE NOTICE '1. elections.is_active column added';
  RAISE NOTICE '2. All 9 parties ensured';
  RAISE NOTICE '3. Simulation artifacts cleaned up';
  RAISE NOTICE '4. Indexes created';
  RAISE NOTICE '5. run_fast_simulation replaced with ALL 9 parties';
  RAISE NOTICE '6. get_state_breakdown_from_results created';
