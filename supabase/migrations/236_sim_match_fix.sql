-- ============================================================
-- NEOP 236 — SIM MATCH FIX
--
-- Symptom: of 14,281 paired verifications, 14,274 became
-- DISCREPANCY (only 7 MATCH) — nothing could ever publish.
--
-- Root cause: migration 232's party-vote allocation jittered on
-- the SUBMISSION id (`hashtext(t.sub_id || party)`), so agent 1 and
-- agent 2 of the SAME polling unit always received different party
-- splits even with identical totals -> every pair compared as a
-- discrepancy -> zero canonical publishes.
--
-- Fix: jitter keys on the POLLING UNIT id, which both agents share.
-- Identical totals + identical weights -> identical largest-remainder
-- allocation -> MATCH. Discrepancies now arise only where designed
-- (the ±5-vote injection on p_discrepancy_rate of PUs and disrupted
-- PUs, which go to HUMAN_REVIEW as before).
--
-- Also:
--   * party_results pass gets the data-chunk hash filter (later
--     chunks no longer re-probe the whole election's submissions).
--   * idx_verifications_election_status narrows the per-chunk
--     AWAITING_DATA comparison scan.
-- ============================================================

CREATE INDEX IF NOT EXISTS idx_verifications_election_status
  ON public.verifications (election_id, status);

DROP FUNCTION IF EXISTS public.neop_sim_wave(TEXT, BIGINT, INT, INT, INT, NUMERIC, UUID, UUID, INT, INT, INT, INT);
...
CREATE OR REPLACE FUNCTION public.neop_sim_wave(
  ...
  p_init_chunk       INT DEFAULT NULL,
  p_init_chunks      INT DEFAULT NULL,
  p_data_chunk       INT DEFAULT NULL,
  p_data_chunks      INT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ----------------------------------------------------------------
  -- INIT CHUNK MODE: mint observers for a hash-slice of PUs only.
  -- Route loops chunks; every call returns in seconds.
  ----------------------------------------------------------------
  IF p_init_chunks IS NOT NULL THEN
    ...
    -- 1) Two simulated observer user_accounts per PU in this chunk
    ...
    WHERE ((hashtext(pu.id::text) & 2147483647) % p_init_chunks) = p_init_chunk
    ON CONFLICT (id) DO NOTHING;
    GET DIAGNOSTICS v_accts = ROW_COUNT;

    -- 2) volunteers: explicit id = user_account id (FKs on
    --    result_submissions/agent_assignments reference volunteers.id)
    ...
    -- 3) Two CHECKED_IN assignments per PU (one observer each, so the
    --    UNIQUE (volunteer_id, election_id) constraint holds by design)
    ...
    v_cfg := jsonb_build_object(
      ...
      'init_chunk', p_init_chunk,
      'init_chunks', p_init_chunks,
      'submitted_total', 0,
      'votes_total', 0,
      'published_total', 0
    );
    ...
    VALUES (p_admin_user_id, 'admin', 'SIM_INIT_CHUNK', 'elections', v_elec,
            jsonb_build_object('chunk', p_init_chunk, 'chunks', p_init_chunks,
                               'accounts', v_accts, 'volunteers', v_vol,
                               'assignments', v_assign_n));
  ...
  ----------------------------------------------------------------
  -- Legacy combined init when wave 0 is invoked without init chunks
  -- (direct SQL callers). The route always uses chunked init instead.
  ----------------------------------------------------------------
  IF v_wave = 0 THEN
    INSERT INTO user_accounts (id, email, full_name, auth_provider)
    SELECT uuid_generate_v5(v_ns, 'neop-sim-obs-' || pu.id::text || '-' || a.role),
  ...
  --    (optionally restricted to a data chunk of that slice)
  ----------------------------------------------------------------
  DROP TABLE IF EXISTS wave_subs;  -- pooling: temp tables can linger on shared server conns
  ...
  --    Jitter keys on the POLLING UNIT (both agents of a PU share it),
  --    so identical totals yield IDENTICAL party splits -> MATCH.
  ----------------------------------------------------------------
  WITH target AS (
  ...
  ----------------------------------------------------------------
  -- 3. Deterministic comparison — trg_rs_timeline paired the agents
  ----------------------------------------------------------------
  DROP TABLE IF EXISTS wave_cmp;  -- pooling: defensive, ON COMMIT DROP normally suffices
  ...
  ----------------------------------------------------------------
  -- 4. PUBLISH through the REAL canonical RPC — MATCH rows only,
  --    filtered BEFORE the LATERAL executes.
  ----------------------------------------------------------------
  -- wave_pub must be explicitly dropped: plain CREATE TEMP TABLE LINGERS on
  -- pooled server connections across RPC calls -> "already exists" failures.
  ...
  ----------------------------------------------------------------
  -- 6. Progress heartbeat (pacing is done by the route, not here)
  ----------------------------------------------------------------
...
GRANT EXECUTE ON FUNCTION public.neop_sim_wave(
  TEXT, BIGINT, INT, INT, INT, NUMERIC, UUID, UUID, INT, INT, INT, INT
) TO service_role;
