-- ============================================================
-- NEOP 231 — SIM WAVE FIX: PER-PU OBSERVER AGENTS + RESET PATTERN
--
-- Wave-0 of neop_sim_wave previously reused a shared 120-agent pool,
-- but live enforces agent_assignments_volunteer_id_election_id_key
-- UNIQUE (volunteer_id, election_id) — one agent may hold only ONE
-- assignment per election, so assignments 2..N per agent violated it
-- and wave 0 rolled back.
--
-- Fix: mint two dedicated simulated observer agents PER POLLING UNIT
-- (role N = observer 1, role S = observer 2) using deterministic
-- uuid_generate_v5 keys, still fully set-based:
--   user_accounts.id  = uuid5('neop-sim-obs-<pu_id>-N'|'-S')
--   volunteers.user_id = same id (unique)
--   agent_assignments.volunteer_id = same id
-- Waves then compute the volunteer uuid directly — no joins needed.
--
-- Also fixes neop_reset_live_data(): legacy debris uses the
-- "[SIM-E2E] ..." naming, which does not match LIKE '[SIM]%'
-- (the ] terminates the literal). Use '[SIM-%' and '[TEST]%' instead,
-- and clean sim observer emails 'sim_obs_%@neop.ng' too.
-- ============================================================
...
BEGIN
  ----------------------------------------------------------------
  -- WAVE 0 = INIT: election + two dedicated observer agents per PU
  -- (deterministic uuid_generate_v5 identities; idempotent re-init)
  ----------------------------------------------------------------
  IF v_wave = 0 THEN
    ...
    -- 1) Two simulated observer user_accounts per PU
    ...
    -- 2) volunteers for those accounts. IMPORTANT: volunteer_id FKs on
    --    result_submissions / agent_assignments reference volunteers.id
    --    (its own PK), so we create each volunteer with an EXPLICIT id
    --    equal to its user_account id — then one uuid identifies the
    --    account, the volunteer, and the assignment's volunteer_id.
    ...
    -- 3) Two CHECKED_IN assignments per PU (respects the
    --    UNIQUE (volunteer_id, election_id) constraint by construction:
    --    every observer belongs to exactly one PU)
