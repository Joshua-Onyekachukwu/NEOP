-- ============================================================
-- NEOP 253 — SIMULATION DISRUPTION SALT FIX
--
-- Bug: the "disrupted" flag in neop_sim_wave used the SAME hash
-- modulus as the coverage slice:
--
--     coverage slice:  hashtext(pu.id) % 100 < v_cov
--     disrupted:       hashtext(pu.id) % 100 < 4
--
-- Any run with coverage_pct <= 4 therefore covered ONLY disrupted
-- PUs: agent 2 never submitted, no verification could pair, and
-- zero results were published (the E2E run at 2% coverage exposed
-- this). At 8% coverage, half of all covered PUs were wrongly
-- disrupted, suppressing published counts.
--
-- Fix: salt the disruption hash ('dis:' prefix) so it is fully
-- independent of the coverage slice. Supersedes the predicate in
-- migrations 232/237; everything else in the function is unchanged.
-- ============================================================
...
BEGIN
  ...
  WITH pick AS (
    SELECT pu.id AS pu_id,
           -- FIX (253): salted hash — disruption is now INDEPENDENT of
           -- the coverage slice. The old predicate reused the coverage
           -- modulus, so coverage <= 4 meant every covered PU was
           -- "disrupted" and agent 2 never filed a report.
