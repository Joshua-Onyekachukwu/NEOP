-- ============================================================
-- NEOP 237 — COVERAGE-TARGETED SIM (disk-safe 20M+ votes)
--
-- Adds p_coverage_pct: submissions run over a HASH SLICE of polling
-- units only. Disk/row cost tracks PU COVERAGE (accounts, volunteers,
-- assignments, submissions are all per-PU), not voter count — so 50%
-- coverage with doubled per-PU turnout still lands 20M+ votes while
-- halving every row count. The 20M/100%-coverage run hit the project
-- disk quota ("No space left on device" during wave 1).
--
-- Also inherits 236 rev 2: pool-safe temp tables (DROP IF EXISTS +
-- ON COMMIT DROP) and statement_timeout = 120s so timed-out calls die
-- server-side before route retries pile on.
-- ============================================================
...
DROP FUNCTION IF EXISTS public.neop_sim_wave(TEXT, BIGINT, INT, INT, INT, NUMERIC, UUID, UUID, INT, INT, INT, INT, INT);
...
DECLARE
  ...
  v_vol       BIGINT := 0;
  v_assign_n  BIGINT := 0;
  v_cov       INT;  -- % of PUs in scope (disk budget); turnout scales up to keep votes on target
BEGIN
...
GRANT EXECUTE ON FUNCTION public.neop_sim_wave(
  TEXT, BIGINT, INT, INT, INT, NUMERIC, UUID, UUID, INT, INT, INT, INT, INT
) TO service_role;
