-- Fast simulation: all work happens inside Postgres in a single call
-- Eliminates thousands of REST API round-trips
...
BEGIN
  ...
  RAISE NOTICE '[fast-sim] Scenario: %, NDC share: %', v_scenario, v_ndc_share;

  -- ── Step 1: Reset ──
  RAISE NOTICE '[fast-sim] Resetting data...';
  ...
  RAISE NOTICE '[fast-sim] % PUs, ~% votes per PU', v_total_pus, v_avg_votes_per_pu;

  -- ── Step 3: Set random initial PU statuses ──
  ...
  -- All in one CTE chain — no network round-trips
  WITH pu_data AS (
    SELECT
      pu.id AS pu_id,
      st.name AS state_name,
      -- Regional vote weights (inline)
      CASE COALESCE(st.name, '')
        ...
        ELSE 'NC'
      END AS region,
      -- Random votes with regional weighting
  ...
  result_insert AS (
    ...
    SELECT
      ...
      NULL,
      NULL,
      ROUND(pd.total_votes * (0.82 + random() * 0.15))::INTEGER,
    ...
    FROM pu_data pd
    RETURNING id, polling_unit_id
  ),
  -- Fix rejected_votes = total - valid
  result_fix AS (
    UPDATE result_submissions rs
    SET rejected_votes = rs.total_votes - rs.valid_votes
  ...
  -- Generate party results for each submission
  vote_data AS (
    SELECT
      ri.id AS result_id,
      pd.region,
      pd.total_votes,
      -- NDC votes (always wins)
    ...
    FROM result_insert ri
    JOIN pu_data pd ON pd.pu_id = ri.polling_unit_id
  )
  ...
  SELECT
    ...
    CASE pi.abbreviation
      WHEN 'NDC' THEN vd.ndc_votes
      WHEN 'APC' THEN vd.apc_votes
      ELSE GREATEST(0, ROUND((vd.total_votes - vd.ndc_votes - vd.apc_votes) *
        (random() * 0.3 + 0.02))::INTEGER)
    END
  FROM vote_data vd
  CROSS JOIN party_ids pi
  WHERE (
    CASE pi.abbreviation
      WHEN 'NDC' THEN vd.ndc_votes
      WHEN 'APC' THEN vd.apc_votes
  ...
  RAISE NOTICE '[fast-sim] Created % results, % party_results', v_results_created, v_pr_created;
...
GRANT EXECUTE ON FUNCTION run_fast_simulation(TEXT, INTEGER, BIGINT) TO service_role;
GRANT EXECUTE ON FUNCTION run_fast_simulation(TEXT, INTEGER, BIGINT) TO anon;

-- Also create a function for status progression ticks (called periodically)
...
BEGIN
  ...
  -- Advance ~30% of eligible PUs to next status
  WITH eligible AS (
