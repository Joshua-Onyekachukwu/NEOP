-- COMBINED FIX: Run this single file in Supabase SQL Editor
-- Fixes: 1) election_type column in elections table
--        2) ward_id redistribution for polling units
--        3) Updated simulation functions with timeout + election_type param

-- ============================================================
-- 1. Add election_type column to elections table if missing
-- ============================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'elections' AND column_name = 'election_type'
  ) THEN
    ALTER TABLE elections ADD COLUMN election_type TEXT;
    -- Backfill from existing type column
    UPDATE elections SET election_type = type::text WHERE election_type IS NULL;
    RAISE NOTICE 'Added election_type column to elections table';
  ELSE
    RAISE NOTICE 'election_type column already exists';
  END IF;
END $$;

-- ============================================================
-- 2. Fix polling unit ward_id assignments (BULK)
-- ============================================================
DO $$
DECLARE
  v_unique_wards integer;
BEGIN
  -- Check if ward_ids are broken (all pointing to same ward)
  SELECT count(DISTINCT ward_id) INTO v_unique_wards
  FROM polling_units WHERE ward_id IN (SELECT id FROM wards);

  IF v_unique_wards < 10 THEN
    RAISE NOTICE 'Ward IDs broken (% unique), redistributing...', v_unique_wards;
    ...
    SELECT count(DISTINCT ward_id) INTO v_unique_wards
    FROM polling_units WHERE ward_id IN (SELECT id FROM wards);
    RAISE NOTICE 'Done! Now % unique ward assignments', v_unique_wards;
  ELSE
    RAISE NOTICE 'Ward IDs already OK (% unique)', v_unique_wards;
  END IF;
END $$;

-- ============================================================
-- 3. Updated simulation functions with timeout + election_type
-- ============================================================
...
BEGIN
  ...
  -- Ensure election exists (use both columns for compatibility)
  ...
  -- Bulk insert results + party results
  WITH pu_data AS (
  ...
  result_insert AS (
    ...
    SELECT pd.pu_id, v_election_id, NULL, NULL,
      ...
      CASE WHEN random() < 0.05 THEN 'VERIFIED' WHEN random() < 0.12 THEN 'RESULT_SUBMITTED' ELSE 'RESULT_SUBMITTED' END,
      now() - (random() * interval '60 days'),
      CASE WHEN random() < 0.05 THEN now() - (random() * interval '30 days') ELSE NULL END
    FROM pu_data pd RETURNING id, polling_unit_id
  ),
  result_fix AS (
    UPDATE result_submissions rs SET rejected_votes = rs.total_votes - rs.valid_votes FROM result_insert ri WHERE rs.id = ri.id RETURNING rs.id
  ),
  party_ids AS (SELECT id, abbreviation FROM parties),
  vote_data AS (
  ...
  SELECT vd.result_id, pi.id,
    CASE pi.abbreviation
      WHEN 'NDC' THEN vd.ndc_votes WHEN 'APC' THEN vd.apc_votes
      ELSE GREATEST(0, ROUND((vd.total_votes - vd.ndc_votes - vd.apc_votes) * (random() * 0.3 + 0.02))::INTEGER)
    END
  FROM vote_data vd CROSS JOIN party_ids pi
  WHERE (CASE pi.abbreviation WHEN 'NDC' THEN vd.ndc_votes WHEN 'APC' THEN vd.apc_votes ELSE GREATEST(0, ROUND((vd.total_votes - vd.ndc_votes - vd.apc_votes) * (random() * 0.3 + 0.02))::INTEGER) END) > 0;

  GET DIAGNOSTICS v_pr_created = ROW_COUNT;
  SELECT count(*) INTO v_results_created FROM result_submissions;

  SELECT jsonb_object_agg(status, cnt) INTO v_status_dist FROM (SELECT status, count(*) AS cnt FROM polling_units GROUP BY status) sub;

  UPDATE simulation_config SET status = 'COMPLETED', last_tick_at = now(), total_results_submitted = v_results_created, updated_at = now() WHERE id = v_config_id;
  ...
  RETURN jsonb_build_object(
    ...
    'total_votes', (SELECT sum(total_votes) FROM result_submissions),
...
-- Also ensure simulation_tick has proper timeout
