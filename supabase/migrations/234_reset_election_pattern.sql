-- ============================================================
-- NEOP 234 — RESET FIX: MATCH '[SIM] ' ELECTIONS + FK-SAFE ORPHANS
--
-- neop_reset_live_data() matched sim elections with LIKE '[SIM-%',
-- but pipeline elections are named '[SIM] Pipeline ...' (closing
-- bracket, space) so they survived reset; deleting sim volunteers
-- then violated agent_assignments_volunteer_id_fkey.
--
-- Fixes:
--   * match sim/test elections by exact prefix: left(name,5)='[SIM]'
--     or left(name,6)='[TEST]' (no LIKE bracket subtleties at all)
--   * belt-and-braces: before deleting sim volunteers, mop up any
--     agent_assignments / result_submissions (+ their party_results)
--     that reference a sim observer account regardless of election
-- ============================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'canonical_deleted', v_can,    'canonical_party_deleted', v_cparty,
