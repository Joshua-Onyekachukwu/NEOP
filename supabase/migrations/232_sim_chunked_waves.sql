-- ============================================================
-- NEOP 232 — CHUNKED SIM WAVE (gateway-timeout-proof)
--
-- Wave 0 previously minted ~353k accounts + ~353k volunteers +
-- ~177k assignments in ONE RPC call. On the hosted gateway that call
-- ran for minutes and was killed with "upstream request timeout",
-- rolling back the whole wave transaction.
--
-- Fix: every heavy step is CHUNKED across multiple small RPC calls
-- (seconds each):
--   * INIT chunks  (p_init_chunks NOT NULL): election + observer
--     accounts/volunteers/assignments for hash-slice of PUs only.
--     Route loops chunks 0..C-1.
--   * DATA chunks  (p_data_chunks NOT NULL): submissions slice for
--     wave w restricted to hash-slice; party_results / pairing /
--     publish steps are naturally incremental (NOT EXISTS + status
--     guards), so repeated chunked calls converge correctly.
-- Pacing (configurable duration) moved to the route: pg_sleep in-RPC
-- held a gateway connection open for the whole duration.
-- ============================================================

DROP FUNCTION IF EXISTS public.neop_sim_wave(TEXT, BIGINT, INT, INT, INT, NUMERIC, UUID, UUID);
...
BEGIN
  ...
  --    an in-RPC pg_sleep would hold the gateway call open)
  ----------------------------------------------------------------
